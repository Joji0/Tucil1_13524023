...
string get_file_dialog(bool save) {
        string cmd;
        string res = "";
        char buf[128];
#ifdef _WIN32
...
void save_img() {
        if (!found || n == 0) return;
        string path = get_file_dialog(true);
...
int main() {
        ...
        Btn b_sv({260.f, 40.f}, {20.f, 660.f}, "SAVE IMG", f, C_BTN_B);
        Text st(f, "Idle", 14);
        ...
        while (w.isOpen()) {
                                ...
                                if (mb->button == Mouse::Button::Left) {
                                        ...
                                        if (!solving) {
                                                if (b_brs.upd(mp, true)) {
                                                        string p = get_file_dialog(false);
                                                        if (!p.empty() && p != "ERROR") {
                                                                file_path = p;
                                                                inp.set(p);
                                                        }
                ...
                if (solving) {
                        ...
                        st.setString("SOLVED\nIter: " + to_string(iter) +
                                     "\nTime: " + to_string((long long)elapsed_time) + "ms");
